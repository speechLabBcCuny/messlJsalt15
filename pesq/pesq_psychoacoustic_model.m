...
if dctSmooth < Nb
    D = dctmtx(Nb);
    smoother = D(1:dctSmooth,:)' * D(1:dctSmooth,:);
    pr = pitch_pow_dens_ref;
    pd = pitch_pow_dens_deg;
    pitch_pow_dens_ref = exp(log(pitch_pow_dens_ref + 1e-6) * smoother') - 1e-6;
    pitch_pow_dens_deg = exp(log(pitch_pow_dens_deg + 1e-6) * smoother') - 1e-6;
    
    subplots(listMap(@(x) max(0, db(x')), {pr, pd, pitch_pow_dens_ref, pitch_pow_dens_deg}))
end
...
for utt = 2: Nutterances
    ...
    if (frame1 > j && j >= 0) %% DSW Added j >= 0 6/25/2012
        frame1 = j;
    elseif (frame1 < 0)
        frame1 = 0;
    end
