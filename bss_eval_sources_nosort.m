function [SDR,SIR,SAR]=bss_eval_sources_nosort(se,s)

% Modified from E. Vincent's bss_eval_sources.m code by J. Le Roux to avoid
% looking at all possible permutations, Sep. 2011.
%
% BSS_EVAL_SOURCES_NOSORT Measurement of the separation quality for
% estimated source signals in terms of filtered true source, interference
% and artifacts.
%
% [SDR,SIR,SAR,perm]=bss_eval_sources_nosort(se,s)
%
% Inputs:
...
for jest=1:nsrc,
    ...
    [SDR(jest),SIR(jest),SAR(jest)]=bss_source_crit(s_true,e_spat,e_interf,e_artif);
end
