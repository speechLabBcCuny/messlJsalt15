...
[y,Fs] = audioread(estFile);
siz = size(y);
for j=1:J
    if any(siz~=size(audioread(srcFiles{j})))
        fprintf(2,'Size of source file %d (%s): %d x %d\n',...
            j,srcFiles{j},audioread(srcFiles{j},'size'));
...
try
    [siz fs] = audioread(srcFiles{1},'size');
